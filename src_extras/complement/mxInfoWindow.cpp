...
void mxInfoWindow::OnButtonOk (wxCommandEvent & evt) {
	...
	if (step==STEP_SHOWING_INFO) {
		...
		if (!CreateDirectories(callback_info,file,dest)) { wxMessageBox(SP("Ha ocurrido un error durante la instalación (CreateDirectories).","There was a error installing the complement (CreateDirectories)")); Finish(); return; }
		if (ShouldStop()) { Finish(); return; }
		step=STEP_CREATING_FILES;
		if (!ExtractFiles(callback_info,file,dest)) { wxMessageBox(SP("Ha ocurrido un error durante la instalación (ExtractFiles).","There was a error installing the complement (ExtractFiles)")); Finish(); return; }
		if (ShouldStop()) { Finish(); return; }
