...
mxInspectionPrint::mxInspectionPrint(wxString expression, bool is_frameless) : wxPanel(main_window,wxID_ANY,wxDefaultPosition,wxDefaultSize) {
	...
	if (s.Len()) {
		// calcular tamaņo de la ventana para el texto que tiene y con la relacion de aspecto a
		// la idea es, con texto en una line tamaņo w*h => defino T=w*h, y quiero w*a=h.. con esas
